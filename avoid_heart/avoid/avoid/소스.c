...
void endGame(int flag)
{
	...
	else // 사용자가 스테이지를 클리어했을 경우
	{
		...
		printf("## 하트의 갯수는 스테이지마다 50개씩 늘어납니다 ##\n");
		gotoxy(MAP_X/2, MAP_Y/2+3);
		printf("## 아무키나 누르면 시작 ##\n");
...
void print_heart(void)
{
	...
	for(count = 0 ; count < HEART ; count ++)
	{
		...
		if(heart[count].y == MAP_Y-1)
		{
			...
			printf("STAGE [ %d ]\n", stage+1);
			printf(">> 피한 하트의 갯수 : %d\n", count_heart);
			printf(">> 남은 하트의 갯수 : %d", remain_heart - count_heart);
...
void check_hit(void)
{
	...
	for(count = 0 ; count < HEART ; count ++)
	{
		...
		{
			// 같은 행인지 확인
			if(heart[count].y == charic.y || heart[count].y == charic.y-1 || heart[count].y == charic.y-2)
			{
				gotoxy(MAP_X, MAP_Y+1);
...
int main(void)
{
	PlaySound(TEXT("C:\\Users\\DSM2018\\Desktop\\phoneNum\\res\\Fade.wav"), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
	// 랜덤 값 초기화
	srand(time(0));
	printf("┌---------------------------------------┐\n");
	printf("│                                       │\n");
